...
export default class AudioBufferSourceNode extends AudioScheduledSourceNode {
  ...
  constructor(context, node, pitchCorrection) {
    ...
    if (pitchCorrection) {
      this.detune = new AudioParam(new IStretcherNodeAudioParam(0, this.setDetune.bind(this), 'a-rate', -1200, 1200, 0));
      this.playbackRate = new AudioParam(new IStretcherNodeAudioParam(1, this.setPlaybackRate.bind(this), 'a-rate', 0, Infinity, 1));
    } else {
  ...
  start(when, offset, duration) {
    ...
    this.asStretcher().start(startAt, offset, duration, this.playbackRate.value, Math.floor(clamp(this.detune.value / 100, -12, 12)));
  }
  stop(when = 0) {
    if (when < 0) {
      throw new RangeError(`when must be a finite non-negative number: ${when}`);
